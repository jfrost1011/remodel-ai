...
class PDFService:
    ...
    async def generate_estimate_pdf(
        ...
        include_similar_projects: bool = True,  # kept for future use
    ) -> str:
        """
        Generate a PDF report for an estimate.

        If the specified estimate_id is not found in the cache, the service now
        ...
        try:
            ...
            cost_data = [
                ...
                [
                    "Confidence Score",
                    f"{estimate_data['confidence_score']*100:.0f}%",
                ],
            ]
            ...
            if include_breakdown:
                ...
                breakdown_data = [
                    ...
                    [
                        "Materials",
                        f"${estimate_data['cost_breakdown']['materials']:,.0f}",
                        "40%",
                    ],
                    [
                        "Labor",
                        f"${estimate_data['cost_breakdown']['labor']:,.0f}",
                        "35%",
                    ],
                    [
                        "Permits",
                        f"${estimate_data['cost_breakdown']['permits']:,.0f}",
                        "5%",
                    ],
                    [
                        "Other",
                        f"${estimate_data['cost_breakdown']['other']:,.0f}",
                        "20%",
                    ],
                ]
                breakdown_table = Table(
                    breakdown_data, colWidths=[2 * inch, 2 * inch, 1.5 * inch]
                )
            ...
            timeline_data = [
                ...
                [
                    "Construction",
                    f"{estimate_data['timeline']['construction_days']} days",
                ],
            ...
            story.append(
                Paragraph(
                    "Generated by RemodelAI - AI-Powered Construction Estimates",
                    footer_style,
                )
            )
            story.append(
                Paragraph(
                    "Estimates are approximations based on historical data", footer_style
                )
            )
            story.append(
                Paragraph(
                    "Valid for 30 days from generation date", footer_style
                )
            )
